// UniPix : Pixel Art Tools by Unitycoder.com
...
namespace unitycodercom_unipix
{

	public class UniPix : EditorWindow 
	{
		
		static string appName = "UniPix";
		...
		// toolbar
		//										 0        1        2       3       4        5        6
	    string[] toolbarStrings = new string[] {"New.."," Brush", "Fill", "Save", "Save+", "Clear", "Settings"};
		...
		// window
		[SerializeField] static UniPix window;
			
		[MenuItem ("Window/UniPix")]
		static void Init () {
			window = (UniPix)EditorWindow.GetWindow (typeof (UniPix));
		...
		void GUIEvents()
		{
			...
			if (current.type == EventType.MouseDrag || current.type == EventType.MouseDown)
			{
				...
				if (x>=0 &&  x < imageSize*pixelSizeAdjust && y>=0 && y < imageSize*pixelSizeAdjust)
				{
					...
					if (!wasModified) {}
					{
						if (!window) window = (UniPix)EditorWindow.GetWindow (typeof (UniPix));
						window.title=appName+"*";
						wasModified = true;
					}
