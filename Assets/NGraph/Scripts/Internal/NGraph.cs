...
public abstract class NGraph : MonoBehaviour
{
   ...
   {
      get { return mResolutionCheck; }
      set { mResolutionCheck = value; }
   }
   public float ResolutionCheckRate
   {
      get { return mResolutionCheckRate; }
      set { mResolutionCheckRate = value; }
   }
   bool mResolutionCheck = true;
   float mResolutionCheckLast = 0;
   float mResolutionCheckRate = 0.1f;
   float mLastScreenWidth = 0;
   float mLastScreenHeight = 0;
   public virtual void Update()
   {
      ...
      mResolutionCheckLast -= Time.deltaTime;
      if(mResolutionCheckLast < 0)
      {
         mResolutionCheckLast = mResolutionCheckRate;
         if(ResolutionCheck && (Screen.width != mLastScreenWidth || Screen.height != mLastScreenHeight))
         {
