...
public class NGraphEditor : Editor
{
   public override void OnInspectorGUI()
   {
      ...
      // Resolution check rate
      b = EditorGUILayout.BeginToggleGroup("Auto redraw on window resize", pGraph.ResolutionCheck);
      if (b != pGraph.ResolutionCheck)
         UndoableAction<NGraph>( gr => gr.ResolutionCheck = b);
      float f = EditorGUILayout.FloatField("Rate:", pGraph.ResolutionCheckRate);
      if (f != pGraph.ResolutionCheckRate)
         UndoableAction<NGraph>( gr => pGraph.ResolutionCheckRate = f );
